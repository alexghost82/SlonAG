...
from acta.safety.errors import ArgValidationError, UnknownToolError
...
_REGISTRY: dict[str, SafetyRule] = {
    ...
    # Wave 25: Unified filesystem tool — single entry point for all file operations.
    # Legacy "file_controller" is kept for backward compat and delegates to "filesystem".
    "filesystem": SafetyRule(
        RiskLevel.CONFIRM,
        schema=ArgSchema(
            required=("action",),
            types=_types(
        ...
        actions=(
            ...
            ("create_directory", RiskLevel.CONFIRM),
            ("create_folder", RiskLevel.CONFIRM),
            ("copy", RiskLevel.CONFIRM),
            ("move", RiskLevel.CONFIRM),
            ("rename", RiskLevel.CONFIRM),
            ("organize_desktop", RiskLevel.CONFIRM),
            ("delete", RiskLevel.EXACT_CONFIRM),
            ("trash", RiskLevel.EXACT_CONFIRM),
            ("remove", RiskLevel.EXACT_CONFIRM),
        ),
    ),
