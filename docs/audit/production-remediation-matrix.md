...
| SLON-020 | P0 | Config | `config/schema.py` | `validate_settings` | ALREADY_FIXED | Voice fields parse and round-trip. | Parser never wired voice keys. | `settings == validate_settings(settings.to_dict())` for all persisted fields. | — | `tests/unit/config/test_schema.py` round-trip | UNIT | DONE — R9 |
| SLON-021 | P1 | CI | `pyproject.toml` | ruff/mypy/coverage omit | PARTIAL | Staged ruff include for `runtime/jobs.py`, `runtime/metrics.py`, `runtime/commands.py`. Legacy excludes remain. | Legacy debt hidden from gate. | Staged enable after each module is clean (R10). Do not enable all at once. | R1–R9 module cleanup | Gate includes cleaned modules | STATIC | DONE staged — R10 |
| SLON-022 | P0 | Runtime / Voice | `main.py`, `runtime/live_session.py` | SlonLive / Gemini Live | PARTIAL | AgentLoop has no Gemini Live import. `VoiceBridge` is the provider-neutral STT/TTS path. SlonLive remains Gemini realtime transport. Do not claim One AgentLoop DONE. | Two reasoning/transport planes. | Do **not** claim “One AgentLoop DONE” while SlonLive still reasons. VoiceBridge / provider-neutral live contract in R7. | SLON-015, SLON-025 | Voice interrupt/barge-in tests | UNIT + MANUAL | DONE VoiceBridge — R7; Live transport leftover |
...
| SLON-025 | P0 | UI | `main.py`, `ui.py`, `ui/*` | desktop glue | PARTIAL | Typed commands + architecture scan: UI does not import AgentLoop/ToolExecutor/Router/Live. Widgets still large; Live still in `main.py`. | Historical monolith. | Typed commands/events. SlonUI/JarvisUI thin clients. UI must not call providers / ToolExecutor / AgentLoop directly. | SLON-022, SLON-024 | Architecture import scan of `ui/` | ARCH | DONE import invariant — R7; widget rewrite leftover |
...
| SLON-033 | P1 | MCP / FS / Browser | `acta/mcp`, `acta/filesystem`, browser actions | policy bypass risk | PARTIAL | MCP CONFIRM no longer auto-approves. FS suite green (macOS `/var`). Web content helper UNTRUSTED. Playwright hardware still missing. | Incremental Wave 14 adapters. | R8 audit: MCP cannot bypass SafetyPolicy; FS `resolve+is_relative_to`; browser content UNTRUSTED. | SLON-003, SLON-005 | Adversarial FS + MCP + injection tests | UNIT | DONE python — R8; Playwright leftover |
| SLON-034 | P1 | Gateway / Sessions | `gateway/*`, `sessions/*`, `server/*` | dual API | PARTIAL | Boundaries documented. Session expire/crash/resume/duplicate/cancel tests added. Gateway reconnect + backpressure + malformed frames covered. iOS hardware still MANUAL. `server/listener` remains local `/v1`. | Legacy `/v1` + first-party gateway. | R6: clarify boundaries; do not weaken TLS/pairing. Session survives disconnect. | SLON-010, SLON-023 | Reconnect / resume / malformed frames | UNIT | DONE python — R6; iOS MANUAL leftover |
