...
| SLON-017 | P0 | Memory | `memory/memory_manager.py` vs `acta/memory/*` | dual stores | VERIFIED_PRESENT | Live path: JSON `memory_manager` + liberal extract. Canonical: `acta/memory` SQLite propose/commit (`repository.py`). `main.py` still uses legacy. `acta/selfimprovement` also imports `memory_manager`. | Two architectures. | One canonical memory: personal / session / working context. Provenance enum. Migrate JSON. Wire live path off legacy once replacement covers extraction. | SLON-009, SLON-018, SLON-029 | Migration + live-path unit | UNIT + INTEGRATION | OPEN — R4 |
| SLON-018 | P0 | Hygiene | `memory/slon_sessions.sqlite3*`, `ui/memory/slon_sessions.sqlite3*` | tracked runtime DBs | VERIFIED_PRESENT | `git ls-files` tracks six sqlite files. User deleted shm/wal in this clone (uncommitted). R0 must **not** `git rm`. | Runtime DBs committed. | Untrack after gitignore (R4). Do not delete fixtures (`tests/unit/memory/fixtures/legacy_memory.json` is JSON, keep). | SLON-019 | `git ls-files` has no runtime sqlite | STATIC | OPEN — R4 |
| SLON-019 | P0 | Hygiene | `.gitignore` | sqlite patterns | VERIFIED_PRESENT at R0 start; **R0 fix** | Pre-R0: `memory/*.db` only. Missing `*.sqlite3`, `-wal`, `-shm`, `memory/gateway_artifacts/`, `ui/memory/`. | Incomplete ignore. | Add patterns in R0. Do not `git rm` tracked DBs in R0. | SLON-018 | gitignore contains patterns | STATIC | R0 gitignore done; untrack deferred |
...
| SLON-023 | P0 | Jobs | *(missing)* | persistent Job Engine | SPEC_GAP | `agent/task_queue.py` is in-RAM (`Task` dataclass, no WAL). No UUID job_id / idempotency_key / next_retry_at / checkpoint. States missing WAITING/RETRYING/PAUSED. | Wave 15 queue ≠ crash-safe jobs. | Canonical Job Engine in R5. Do not build it in R2. | SLON-010, SLON-024 | Restart/recovery tests | UNIT + INTEGRATION | OPEN — R5 |
| SLON-024 | P0 | Events | `runtime/events.py` | `RuntimeEventBus` | PARTIAL | Typed fan-out exists (`RuntimeEventKind`: listening/thinking/tool_*/speaking/cancelled). Missing: `event_id`, `event_type` catalog for UI commands, `job_id`, `correlation_id`, `schema_version`, persistence, replay, backpressure policy beyond isolated subscribers. | First-cut bus, not the R5/R7 contract. | **Extend** this bus. Do not create a second bus. | SLON-023, SLON-025 | Ordering / subscriber failure / replay tests | UNIT | OPEN — R5 / R7 |
...
| SLON-029 | P0 | Memory | `acta/memory/repository.py` | `MemoryRecord` | PARTIAL | Has `source`, `confidence`, `session_id`, `workspace`, `dedup_hash`. **No** provenance enum `USER_EXPLICIT / USER_INFERRED / TOOL_OBSERVATION / SYSTEM_FACT / IMPORTED / ASSISTANT_UNVERIFIED`. No TTL / supersession policy in the record. Injector must treat retrieval as DATA. | Propose/commit store is v2 scoping, not full provenance model. | Add provenance; ASSISTANT_UNVERIFIED must not auto-promote to trusted personal fact. | SLON-017, SLON-009 | Unit: unverified cannot become trusted | UNIT | OPEN — R4 |
...
| SLON-037 | P1 | Automation | `acta/proactive`, `acta/automation`, `acta/workflow_learning` | three engines | PARTIAL | Three subsystems. No single persistent Job Engine tying them. | Parallel feature waves. | R5: call-graph; pick ONE canonical implementation; migrate; then remove duplicates. | SLON-023 | Schedule/DST/restart/dedup tests | UNIT + INTEGRATION | OPEN — R5 |
