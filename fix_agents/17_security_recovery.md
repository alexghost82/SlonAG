# Агент 17 — Security / Recovery Audit Fixes
...
Проверить security/recovery после последних shell/filesystem изменений.
...
- shell security validation tests
- filesystem security validation tests

Минимальные production fixes разрешены только в security-related коде.

## Проверить

- path traversal;
- symlink escape;
- Unicode shell bypass;
- subshell;
- separators;
...
Не ослаблять ограничения ради тестов.

## Acceptance criteria

- Security bypass regression tests зелёные.
- Recovery не оставляет небезопасное состояние.
