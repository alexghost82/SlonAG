...
GatewayHandler = Callable[[GatewayContext, GatewayEnvelope], GatewayEnvelope | Awaitable[GatewayEnvelope]]
...
class GatewayRouter:
    ...
    async def dispatch(self, context: GatewayContext, envelope: GatewayEnvelope) -> GatewayEnvelope:
...
def response_envelope(request: GatewayEnvelope, response_type: str, payload: dict[str, object]) -> GatewayEnvelope:
...
def bind_session_routes(router: GatewayRouter, manager) -> None:
    ...
    def session_list(context: GatewayContext, request: GatewayEnvelope) -> GatewayEnvelope:
        ...
        return response_envelope(
            ...
            {
                "sessions": [
                    {"id": item.id, "title": item.title, "status": item.status.value, "updated_at": item.updated_at}
    ...
    def lifecycle(method: str, response_type: str) -> GatewayHandler:
        def execute(context: GatewayContext, request: GatewayEnvelope) -> GatewayEnvelope:
            item = getattr(manager, method)(require_id(request), workspace_id=context.workspace_id)
            return response_envelope(request, response_type, {"id": item.id, "status": item.status.value})
