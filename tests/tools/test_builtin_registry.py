...
def test_builtin_registry_contains_each_migrated_tool_once() -> None:
    ...
    assert legacy_not_in_builtins <= {"cmd_control", "stt_listen", "tts_speak"}, (
        f"Unexpected legacy-only handlers: {legacy_not_in_builtins}"
    )
...
def test_builtin_schema_is_derived_from_safety_validation_knowledge() -> None:
    ...
    for spec in registry.list():
        ...
        assert properties == {name: {"type": json_type[type_name]} for name, type_name in rule.schema.types}
