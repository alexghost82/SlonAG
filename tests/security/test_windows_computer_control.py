...
class TestShellMetacharacterInjection:
    ...
    def test_injection_rejected(self, payload: str) -> None:
        assert _validate_windows_launch_input(payload) is None, f"FAILED to reject: {payload!r}"
...
class TestEnvVarExpansion:
    ...
    def test_env_var_rejected(self, payload: str) -> None:
        assert _validate_windows_launch_input(payload) is None, f"FAILED to reject env-var: {payload!r}"
...
class TestCmdPwshInvocation:
    ...
    def test_cmd_powershell_blocked(self, payload: str) -> None:
        assert _validate_windows_launch_input(payload) is None, f"FAILED to reject cmd/pwsh: {payload!r}"
...
class TestBlockedBasenames:
    ...
    def test_blocked_basename_rejected(self, name: str) -> None:
        assert _validate_windows_launch_input(name) is None, f"FAILED to reject basename: {name!r}"
    ...
    def test_safe_paths_accepted(self, path: str) -> None:
        assert _validate_windows_launch_input(path) == path, f"Falsely blocked: {path!r}"
...
class TestUnicodeHomoglyphs:
    ...
    def test_unicode_rejected(self, payload: str) -> None:
        ...
        assert result is None, f"FAILED to reject unicode evasion: {payload!r}"
...
class TestEndToEnd:
    ...
    def test_none_input_returns_none(self) -> None:
        ...
        assert _validate_windows_launch_input(None) is None
...
class TestRegexRegression:
    ...
    def test_injection_regex(self, pattern: str) -> None:
        assert _WINDOWS_INJECTION_RE.search(pattern), f"Injection regex missed: {pattern!r}"
    ...
    def test_cmd_regex(self, pattern: str) -> None:
        assert _WINDOWS_CMD_INVOCATION_RE.search(pattern), f"Cmd regex missed: {pattern!r}"
