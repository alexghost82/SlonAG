...
class TestI18nScan:
    ...
    def test_i18n_keys_match(self, project_root: Path) -> None:
        ...
        assert en_keys == ru_keys, (
            f"Top-level keys differ: only in en={en_keys - ru_keys}, only in ru={ru_keys - en_keys}"
        )
    ...
    def test_no_fabricated_keys(self, project_root: Path) -> None:
        ...
        for src_file in sorted(project_root.glob("**/*.py")):
            ...
            if any(
                p in (".venv", ".git", "__pycache__", "node_modules", "build", "dist", "tests", "i18n", ".pytest_cache")
                for p in parts
            ):
                continue
            try:
        ...
        assert bad_keys == [], f"Keys used in t() not found in i18n: {', '.join(bad_keys[:10])}"
    ...
    def test_actions_have_t_import(self, project_root: Path) -> None:
        ...
        assert missing == [], f"Action files with user-facing output but no i18n import: {', '.join(missing)}"
    ...
    def test_standalone_have_t_import(self, project_root: Path) -> None:
        ...
        for name in standalone:
            ...
            if not _file_has_t_import(p) and _has_user_facing_output(p):
                missing.append(name)
        assert missing == [], f"Standalone files with user-facing output but no i18n import: {', '.join(missing)}"
    ...
    def test_all_keys_exist_in_ru(self, project_root: Path) -> None:
        ...
        assert missing == [], f"Keys missing from ru.json: {', '.join(missing[:10])}"
