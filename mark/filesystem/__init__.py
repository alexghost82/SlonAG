...
Usage::
    ...
    from mark.filesystem import security, operations, tools

    # Validate a path
    from mark.filesystem.security import validate_path, default_allowlist_roots
    roots = default_allowlist_roots()
    resolved = validate_path("my_file.txt", roots)

    # Perform an operation
    from mark.filesystem.operations import read, write, filesystem_operation
    result = read("my_file.txt", roots=roots)

    # Register in tool registry
    from mark.filesystem.tools import UNIFIED_FILESYSTEM_TOOL, READ_FILE_TOOL, FILE_CONTROLLER_TOOL
